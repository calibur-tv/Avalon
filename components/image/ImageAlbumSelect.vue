...
<template>
  ...
  >
    ...
    >
      ...
      <div v-if="item.is_creator" class="is-creator meta">
        创
      </div>
      <div v-if="item.is_cartoon" class="is-cartoon meta">
        漫
      </div>
      <div class="info">
...
<script>
import { getUserAlbums } from '~/api/imageApi'
...
export default {
  ...
  computed: {
    selectionAlbum() {
      return this.append.concat(this.albums)
    }
  },
  mounted() {
  ...
  methods: {
    async getUserAlbum() {
      ...
      this.loading = true
      try {
        this.albums = await getUserAlbums(this)
      } finally {
        this.loading = false
      }
    }

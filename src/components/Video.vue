  ...
  <div class="vue-pwa-video"
    ...
    <div class="vue-pwa-video-box"
      ...
      <div class="vue-pwa-video-waiting"
           v-show="state.waiting"></div>
      <div class="vue-pwa-video-error"
           v-show="state.error"></div>
      <div class="vue-pwa-video-debug"
           v-if="debug">
        <p v-for="(txt,index) in logs" :key="index"
           v-text="txt"></p>
      </div>
    </div>
    <capture-button @pause="video.pause()" :target="$refs.video"></capture-button>
    <transition name="fade">
      <div v-show="state.showTool"
        ...
        <a v-if="next"
           class="btn vue-pwa-video-btn-next"
           :href="next"></a>
        ...
        <div class="vue-pwa-video-progress-bar">
          <v-range v-model="value.playing"
                   ...
                   :max="value.duration"
                   :disabled="otherSrc"
                   @rangeChangeEvent="handleSeek"></v-range>
        </div>
        <div class="vue-pwa-video-time">
...
export default {
  ...
  methods: {
    ...
    tool(e) {
      if (
        this.state.isFull &&
        !this.state.showTool &&
        document.body.offsetHeight - e.screenY < 40
      ) {
  ...
  mounted() {
    ...
    video.addEventListener('stalled', function() {
      self.debugLog(
        'stalled : 浏览器尝试获取媒体数据，但数据不可用时 | 浏览器不论何种原因未能取回媒介数据'
      )
    })
